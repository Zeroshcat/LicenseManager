...
**设备UUID说明**：
- **Linux**: 使用 DMI 主板序列号（优先读取 `/sys/class/dmi/id/board_serial`，必要时调用 `dmidecode`）
- **macOS**: 使用 `system_profiler SPHardwareDataType` 获取整机序列号
- **Windows**: 使用 PowerShell/CIM 或 `wmic baseboard` 获取主板序列号
- 不使用 `/etc/machine-id`、主机名、MAC地址等系统安装态或网络态标识；无法获取可信硬件序列号时会直接报错
...
#### 设备UUID方案
详细的设备UUID生成方案和使用说明，请参考 [docs/DEVICE_UUID_CN.md](docs/DEVICE_UUID_CN.md)
...
A: 设备UUID基于主板/整机硬件序列号生成，标准UUID格式。可以通过以下方式获取：
```bash
# 使用命令行工具（推荐）
./licensemanager uuid
# 或
...
1. **只使用稳定硬件序列号**：
   ...
   - macOS: 整机硬件序列号
   - Windows: 主板序列号
2. **失败即报错**：如果无法获取可信硬件序列号，不会降级使用 `/etc/machine-id`、主机名、MAC地址等可变标识
